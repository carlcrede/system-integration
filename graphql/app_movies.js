...
const ActorType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        movies: {
            ...
            description: 'List of movies this actor stars in',
            resolve: (actor) => {
...
const MovieType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        actors: {
            ...
            description: 'List of all actors in the movie',
            resolve: (movie) => {
...
const RootMutationType = new GraphQLObjectType({
    ...
    fields: {
        addMovie: {
            ...
            resolve: (parent, args) => {
                const foundActors = actors.filter(actor => args.actorIds.includes(actor.id));  
                if (foundActors.length !== args.actorIds.length) {
                    throw new Error('Invalid actor id');
                }
                const movie = { id: movieId++, title: args.title, actorIds: args.actorIds };
                movies.push(movie);
                return movie;
            }
        },
        addActor: {
            ...
            resolve: (parent, args) => {
                const actor = { id: actorId++, name: args.name };
                actors.push(actor);
                return actor;
            }
        }
